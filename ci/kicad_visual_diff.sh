...
HTML="$OUT/index.html"
cat > "$HTML" <<H
<!doctype html><meta charset=utf-8><title>KiCad görsel diff $BASESHA → $HEADSHA</title>
...
<h1>Görsel diff: <code>$BASESHA</code> → <code>$HEADSHA</code></h1>
<p>Sol: önce · Orta: sonra · Sağ: fark (<span style="color:#f55">kırmızı</span> = değişen yerler)</p>
H
render_sch(){ # proje-dizini çıktı-dizini
...
for DIR in "$@"; do
  ...
  for side in sch pcb; do
    for h in "$W/head_$side"/*.svg; do
      ...
      if [ "$px" != "0" ]; then changed=1
        echo "<h3>${side^^} · $f <small>($px piksel değişti)</small></h3><div class=row>" >> "$HTML"
  ...
  [ $changed = 0 ] && echo "<p class=none>Görsel fark yok.</p>" >> "$HTML"
